...
func TestGetGuide(t *testing.T) {
	...
	tests := []struct {
		...
		parserErr    error
		expectErr    bool
		expectGuide  model.ViaGuide
		parserGuide  model.ViaGuide
		responseBody string
		statusCode   int
	}{
		{
		...
		},
		{
			...
			parserGuide:  model.ViaGuide{ID: "123"},
			mockResp: &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(bytes.NewBufferString("html...")),
			},
			expectGuide: model.ViaGuide{ID: "123"},
		},
		{
			name:         "success",
			statusCode:   http.StatusOK,
			responseBody: "<html>...</html>",
			parserGuide:  model.ViaGuide{ID: "999"},
			mockResp: &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(bytes.NewBufferString("<html>...</html>")),
			},
			expectGuide: model.ViaGuide{ID: "999"},
		},
	}
